...
docker compose --env-file .env -f infra/docker-compose.yml up -d mysql kafka-1 kafka-2 kafka-3 kafka-exporter kafka-ui mysql-exporter
docker compose --env-file .env -f infra/docker-compose.yml up -d --build waito-consumer waito-api
docker compose --env-file .env -f infra/docker-compose.yml up -d redis redis-exporter
```
